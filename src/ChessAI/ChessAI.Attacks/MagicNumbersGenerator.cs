...
public sealed class MagicNumbersGenerator
{
    ...
    private ulong FindBishopMagicNumberForSquare(int square)
    {
        ...
        for (var randomCount = 0; randomCount < 100000000; ++randomCount)
        {
            ...
            {
                if (failed) break;
                var magicIndex = Convert.ToInt32(((occupancies[index] * magicNumber) >> (Board.NumberOfSquares - relevantBits)).Value);
    ...
    {
        var bishopMagicNumbers = new ulong[Board.NumberOfSquares];
        var rookMagicNumbers = new ulong[Board.NumberOfSquares];
