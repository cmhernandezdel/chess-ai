...
/// </summary>
public sealed class Bitboard(ulong value)
{
    ...
    // -- BITWISE OPERATORS --
    public byte GetBit(Square square) => GetBit((int)square);
    public void SetBit(Square square) => SetBit((int)square);
    public void UnsetBit(Square square) => UnsetBit((int)square);
    ...
    public Bitboard SetOccupancy(int index, int bitsInMask)
    {
        ...
        for (var i = 0; i < bitsInMask; ++i)
        {
            ...
            attackMaskCopy.UnsetBit((Square)square);
            if (indexBitboard.GetBit((Square)i) == 1)
            {
                occupancy.SetBit((Square)square);
            }
        }
    ...
    public override string ToString()
    {
        ...
        for (int rank = 0; rank < Board.NumberOfRanks; rank++)
        {
            for (int file = 0; file < Board.NumberOfFiles; file++)
            {
                int square = rank * Board.NumberOfRanks + file;
                if (file == 0) sb.Append($"  {Board.NumberOfRanks - rank} ");
